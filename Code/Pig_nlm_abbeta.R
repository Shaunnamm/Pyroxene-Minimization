...
# library(glmulti)
# multi_a <- glmulti(a ~ Mg + Ca + I(Ca^2) + I(Mg^2) + I(Mg*Ca) + I(Ca^3) + I(Mg^3) + I(Mg^2*Ca) + I(Mg*Ca^2), data = pig,level = 1, )
# print(multi_a)
# 
# multi_b <- glmulti(b ~ Mg + Ca + I(Mg^2) + I(Ca^2) + I(Mg*Ca) + I(Ca^3) + I(Mg^3) + I(Mg^2*Ca) + I(Mg*Ca^2), data = pig,level = 1, )
# print(multi_b)
# 
# multi_beta <- glmulti(beta ~ Mg + Ca + I(Ca^2) + I(Mg^2) + I(Mg*Ca) + I(Ca^3) + I(Mg^3) + I(Mg^2*Ca) + I(Mg*Ca^2), data = pig,level = 1, )
# print(multi_beta)
...
mars_pig=read.delim("/Users/smmorrison/Desktop/R/CheMin/mars_pig_2.txt")
...
for (i in 1:nrow(mars_pig)) {
  f <- function(par) { 
    (((pig$a[i]-(c0_a + c1_a*par[1] + c2_a*par[2] + c3_a*par[2]^2 + c4_a*par[1]*par[2] + c5_a*par[1]^2*par[2]))/weight_a[i])^2
     + ((pig$b[i]-(c0_b + c1_b*par[1] + c2_b*par[1]^2 + c3_b*par[2]^2 + c4_b*par[1]^2*par[2]))/weight_b[i])^2
     + ((pig$beta[i]-(c0_beta + c1_beta*par[1] + c2_beta*par[2] + c3_beta*par[1]^2 + c4_beta*par[1]*par[2] + c5_beta*par[1]^3 + c6_beta*par[1]^2*par[2] + c7_beta*par[1]*par[2]^2))^2))
  }
  ...
  mars_a_calc = append(mars_a_calc, (c0_a+c1_a*mars_Mg_calc[i]+c2_a*mars_Mg_calc[i]^2+c3_a*mars_Mg_calc[i]*mars_Ca_calc[i]+c4_a*mars_Mg_calc[i]^3+c5_a*mars_Ca_calc[i]^3+c6_a*mars_Mg_calc[i]^2*mars_Ca_calc[i]+c7_a*mars_Mg_calc[i]*mars_Ca_calc[i]^2))
  mars_b_calc = append(mars_b_calc, (c0_b+c1_b*mars_Ca_calc[i]+c2_b*mars_Mg_calc[i]^2+c3_b*mars_Mg_calc[i]*mars_Ca_calc[i]+c4_b*mars_Mg_calc[i]^3+c5_b*mars_Ca_calc[i]^3+c6_b*mars_Mg_calc[i]^2*mars_Ca_calc[i]))
  mars_beta_calc = append(mars_beta_calc, (c0_beta+c1_beta*mars_Mg_calc[i]+c2_beta*mars_Ca_calc[i]+c3_beta*mars_Mg_calc[i]^2+c4_beta*mars_Mg_calc[i]*mars_Ca_calc[i]+c5_beta*mars_Mg_calc[i]^3+c6_beta*mars_Ca_calc[i]^3+c7_beta*mars_Mg_calc[i]^2*mars_Ca_calc[i]))
